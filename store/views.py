...
def edit_product_details(request, id):
    ...
    if request.method == 'POST':
        ...
        # book_img = request.FILES['productImage']
        # product.image.update(image=book_img)
...
def category_list(request, category_slug):
    ...
    seller_posted_category_books = Product.objects.filter(category=category, created_by=request.user, is_active=True)
    return render(request, 'store/category.html', {'category': category, 'products': products,
                                                   'seller_posted_category_books': seller_posted_category_books})
